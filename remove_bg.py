...
def run_inference(interpreter, image):
    ...
    print("Output shape:", output_data.shape)  # 打印输出形状
    mask = output_data[0,:,:,0]  # 假设我们关注的类别是第一个通道
    cv2.imwrite('mask_output.jpg', mask * 255)  # 保存遮罩查看
    return mask
...
def display_and_save_image(original_img, mask):
    ...
    mask = (mask > 0.5).astype(np.uint8)  # 阈值处理生成遮罩

    green_background = np.zeros_like(original_img)
    green_background[:, :, 1] = 255  # 设置绿色通道为最大值

    segmented_image = np.where(mask[..., None] == 1, original_img, green_background)
