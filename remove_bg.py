...
def display_and_save_image(original_img, mask, raw_mask):
    # 保存原始模型输出的第一个通道，作为灰度图查看
    first_channel = raw_mask[:, :, 0]
    normalized_mask = (first_channel * 255 / np.max(first_channel)).astype(np.uint8) if np.max(first_channel) > 0 else first_channel
    ...
    # 尝试不同的阈值以找到最佳效果
    mask = (mask > 0.1).astype(np.uint8) * 255  # 降低阈值以捕获更多的前景细节
